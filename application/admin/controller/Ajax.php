...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        if ($splInfo) {
            $ret = $this->fileUpload($uploadDir . $splInfo->getSaveName());
            if($ret['code'] == 1){
                $params = array(
                ...
                $attachment->save();
                @unlink($uploadDir . $splInfo->getSaveName());
    ...
    protected function fileUpload($file){
        ...
        $credentials = new \Aws\Credentials\Credentials('AKIAIIQ6PXY7DO6FZ6CQ', '+Fk/eZcszuA59/JftsE/3D+YrMWzgMmzPinJfXw2');
        //s3客户端
        ...
        //存储桶 获取AWS存储桶的名称
        $bucket = config('aws.bucket');//'AWS存储桶名称';
        //需要上传的文件
        ...
        try {
            ...
            $data = [
                'code' => '1',
                'data' => config('aws.domain').urldecode($result['Key'])
            ];
